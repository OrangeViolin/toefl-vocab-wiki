...
export function expandWikilinks(html, tokens, opts) {
  const { selfId, resolve, onBroken, escape, t } = opts;
  return html.replace(PH_RE, (_, idxStr) => {
    ...
    // 支持 [[PageId$anchor|display]] 和 [[PageId#anchor|display]]（向后兼容）
    // $ 优先 (RFC-memex-0006)
    let sepIdx = target.indexOf('$');
    if (sepIdx <= 0) sepIdx = target.indexOf('#');
    const pageTarget  = sepIdx > 0 ? target.slice(0, sepIdx) : target;
    const anchorFrag  = sepIdx > 0 ? target.slice(sepIdx + 1) : '';
    const anchorSuffix = anchorFrag ? '$' + encodeURIComponent(anchorFrag) : '';

    const resolved = resolve(pageTarget);
    const relAttr = relation ? ` data-rel="${escape(relation)}"` : '';
    if (!resolved) {
      onBroken(pageTarget);
      return `<a class="wikilink broken" href="#${encodeURIComponent(pageTarget)}${anchorSuffix}"` +
        ` data-target="${escape(pageTarget)}"${relAttr}` +
        ` title="${t?.('wl_unresolved') ?? 'Unresolved'}: ${escape(pageTarget)}">${escape(display)}</a>`;
    }
    const [pid] = resolved;
    let cls = pid === selfId ? 'wikilink self' : 'wikilink resolved';
    if (relation) cls += ' semantic';
    return `<a class="${cls}" href="#${encodeURIComponent(pid)}${anchorSuffix}"${relAttr}>${escape(display)}</a>`;
  });
}
