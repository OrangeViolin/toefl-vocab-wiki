 ...
 */
function renderSeeAlsoList(items, resolve, t) {
  const links = items.map((item) => {
    ...
    const title = resolved ? '' : ` title="${t?.('not_found') ?? 'Unresolved'}: ${esc(target)}"`;
  ...
  const heading = t?.('see_also') ?? 'See also';
  return `<section class="sealso-section" aria-label="${esc(heading)}">
    <h2>${esc(heading)}</h2>
    <ul>${links.join('')}</ul>
  </section>`;
}
...
export default {
  ...
  async init(core) {
    ...
    core.hooks.onAfterRender.add(async (html, ctx) => {
      ...
      const seeAlsoHtml = renderSeeAlsoList(rawList, resolve, t);
