...
function renderRouteMap(containerId, places, title) {
  loadLeaflet().then(() => {
    ...
    if (missing.length) {
      ...
      if (legendEl) {
        const missingLabel = window._routeMapT?.('map_missing_coords') ?? 'Missing coords';
        legendEl.textContent = missingLabel + '：' + missing.map(p => `${p.idx}.${p.name}`).join('、');
      }
    }

    if (withCoords.length < 1) {
      const noCoordLabel = window._routeMapT?.('no_coord') ?? 'No coordinate data';
  ...
  }).catch(e => console.error('[route-map] Leaflet load failed:', e));
}
...
export default {
  ...
  async init(core, { COORDS_URL = DEFAULT_COORDS_URL } = {}) {
    window._routeMapT = k => core.t?.(k) ?? k;
    // 预加载坐标数据
    fetch(COORDS_URL)
      .then(r => r.json())
      .then(data => { _placeCoords = data; })
      .catch(e => console.warn('[route-map] Coordinate data load failed:', e));
