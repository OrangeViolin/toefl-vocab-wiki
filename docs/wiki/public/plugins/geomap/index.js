...
const DEFAULT_COORDS_URL = 'data/place_coords.json';
...
function renderGeomap(containerId, pins, title) {
  loadLeaflet().then(() => {
    ...
    if (missing.length) {
      ...
      if (legendEl) {
        const missingLabel = window._geomapT?.('map_missing_coords') ?? 'Missing coords';
        legendEl.textContent = missingLabel + '：' + missing.map(p => p.label ? `${p.label}(${p.place})` : p.place).join('、');
      }
    }

    if (!withCoords.length) {
      const noCoordLabel = window._geomapT?.('no_coord') ?? 'No coordinate data';
      el.innerHTML = `<p style="color:#999;padding:8px">${noCoordLabel}</p>`;
      return;
    }
  ...
  }).catch(e => console.error('[geomap] Leaflet load failed:', e));
}
...
export default {
  ...
  async init(core, { COORDS_URL = DEFAULT_COORDS_URL } = {}) {
    window._geomapT = k => core.t?.(k) ?? k;
    fetch(COORDS_URL)
      .then(r => r.json())
      .then(data => { _placeCoords = data; })
      .catch(e => console.warn('[geomap] Coordinate data load failed:', e));
